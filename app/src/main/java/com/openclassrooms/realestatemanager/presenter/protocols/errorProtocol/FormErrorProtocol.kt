...
interface FormErrorProtocol : ErrorProtocol {
    ...
    override fun onReceiveError(exception: Throwable) {
        when (exception.toDomainExceptionType() as? DomainFormException) {
            ...
            is DomainFormException.WrongCityFormat -> onReceiveWrongCityFormatError()
            is DomainFormException.WrongCountryFormat -> onReceiveWrongCountryFormatError()
            is DomainFormException.WrongDescriptionFormat -> onReceiveWrongDescriptionFormatError()
            is DomainFormException.WrongHouseNumberFormat -> onReceiveWrongHouseNumberFormatError()
            is DomainFormException.WrongPostalCodeFormat -> onReceiveWrongPostalCodeFormatError()
            is DomainFormException.WrongPriceFormat -> onReceiveWrongPriceFormatError()
            is DomainFormException.WrongRoadFormat -> onReceiveWrongRoadFormatError()
